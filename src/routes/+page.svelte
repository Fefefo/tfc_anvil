<script lang="ts">
	import { getWorlds, createWorld, getMetals, createMetal, createItem } from './data.remote';
	...
	let worldValue = $state(worldQuery.current?.length === 1 ? worldQuery.current[0].id : '');
	let metalValue = $state('');
	let itemName = $state('');
...
<main class="flex h-screen w-screen flex-col items-center justify-center gap-4 bg-gray-800">
	...
	<section class="flex flex-col gap-y-4 rounded-2xl bg-white p-6 text-black">
		<input bind:value={itemName} class="rounded" placeholder="Item name" />
		<div class="flex">
	...
	<div class="w-1/4">
		<div class="rounded bg-green-700 py-2 text-center font-black text-amber-50 uppercase">
			<button class="uppercase" onclick={itemCreation}>Create Item</button>
		</div>
	</div>
</main>
